...
func type_out() -> void:
	...
	while is_typing:
		...
		line_count = get_visible_line_count()
...
func _should_auto_pause() -> bool:
	...
	if parsed_text[visible_characters - 1] == ".":
		if parsed_text[visible_characters] == "." or parsed_text[visible_characters - 2] == ".":
			return false
...
signal line_count_changed

var line_count : int = 1:
	set(value):
		if line_count != value:
			line_count = value
			emit_signal("line_count_changed")
