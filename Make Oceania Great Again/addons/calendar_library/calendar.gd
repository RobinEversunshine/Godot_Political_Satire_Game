...
## [br][br]
## The Calendar library comes with a handy DateLibrary class (see [Calendar.DateLibrary]),
...
## Returns the current date as a DateLibrary object.
func get_today() -> DateLibrary:
	var today = Time.get_date_dict_from_system()
	return DateLibrary.new(today.year, today.month, today.day)
...
## month is also an array of DateLibrary objects corresponding to each day in that month.
## [br][br]
...
## Returns an array of weeks, where each week is an array of DateLibrary objects 
	...
	while day <= days_in_month or (force_six_weeks and calendar.size() < 6):
		for i in range(7):
			if day > 0 and day <= days_in_month:
				week[i] = DateLibrary.new(year, month, day)
				...
				week[i] = DateLibrary.new(adj_year, adj_month, adj_day)
			else:
				week[i] = 0
...
## Returns an array of DateLibrary objects for each day in the week containing 
...
@warning_ignore("int_as_enum_without_cast")
func get_calendar_week(year: int, month: int, day: int, days_in_week: int = 7) -> Array[DateLibrary]:
	var dates : Array[DateLibrary] = []
	...
	for i in range(days_in_week):
		...
		var date = DateLibrary.new(current_year, current_month, current_day)
		dates.append(date)
		current_day += 1
...
func get_week_number(year: int, month: int, day: int) -> int:
	match week_number_system:
		WeekNumberSystem.WEEK_NUMBER_FOUR_DAY:
			...
			var date: DateLibrary = DateLibrary.new(year, month, day)
			...
			var first_majority_day_of_year: DateLibrary = DateLibrary.new(year, 1, 1)
			...
			if date.is_before(first_majority_day_of_year) and shifted_weekday in [1, 2, 3]:
				return 1
			elif date.is_after(DateLibrary.new(date.year, 12, 28)) and shifted_weekday in [1, 2, 3]:
				return 1
			else:
		...
		WeekNumberSystem.WEEK_NUMBER_TRADITIONAL:
			var date: DateLibrary = DateLibrary.new(year, month, day)
			var start_of_year = DateLibrary.new(year, 1, 1)
			
			# Handling the end of the year
			if date.month == 12:
				var days_in_december = (DateLibrary.new(year, 12, 31)).get_day_of_year()
			...
			# If the remaining days in December are less than a week and January 1st is not the first day of the week
				var jan_first_day_of_week = (DateLibrary.new(year + 1, 1, 1)).get_weekday_iso()
...
## Returns an array of DateLibrary objects for a number of days, defined by [param days],
...
## in the range.
func get_days_of_range(days: int, year: int, month: int, day: int, exclusive: bool = false) -> Array[DateLibrary]:
	var days_range: Array[DateLibrary] = []
	var total_days: int = days - 1 if exclusive else days
	
	for _i in range(total_days):
		var date = DateLibrary.new(year, month, day)
...
## [b]%F[/b] - DateLibrary in ISO8601 standard format (e.g., 2023-02-05)[br]
## [br]
...
## by the current CalendarLocale's DateLibrary Format and Divider Symbol.
## [br][br]
...
##
## DateLibrary stores data about a specific date, encompassing the year, month, and
...
class DateLibrary:
	...
	func _validate():
		var valid: bool = true
		var error_msg = "DateLibrary is not valid (%s, %s, %s): " % [year, month, day]
		if month < 1 or month > 12:
	...
	## Returns [code]true[/code] if this DateLibrary is before the provided date.
	func is_before(date: DateLibrary) -> bool:
		if year < date.year:
			return true
		elif year == date.year:
			if month < date.month:
	...
	## Returns [code]true[/code] if this DateLibrary is after the provided date.
	func is_after(date: DateLibrary) -> bool:
		if year > date.year:
			return true
		elif year == date.year:
			if month > date.month:
	...
	## Returns [code]true[/code] if this DateLibrary is the same as the provided date.
	func is_equal(date: DateLibrary) -> bool:
	...
	## Set the year, month and day of this DateLibrary. Throws an error if the 
	...
	## Set this DateLibrary to today's date
	func set_today():
	...
	## Set this DateLibrary to the given dictionary's date. The
	...
	## Returns a new DateLibrary object which is a copy of this DateLibrary.
	func duplicate() -> DateLibrary:
		return DateLibrary.new(year, month, day)
	
	
	# Helper function to calculate how many days are between two DateLibrary objects
	@warning_ignore("integer_division")
	...
	## Return the number of days between two DateLibrary objects. Is only accurate
	## when dates are after the year 1582.
	func days_to(date: DateLibrary) -> int:
		return self._to_julian_day() - date._to_julian_day()
	
	
	## A static function that returns a DateLibrary object with todays date.
	...
	## [/codeblock]
	static func today() -> DateLibrary:
		var date: DateLibrary = DateLibrary.new(1, 1, 1)
		#var today_date: Dictionary = Time.get_date_dict_from_system()
		var today_date: Dictionary = DateTime.date_time
